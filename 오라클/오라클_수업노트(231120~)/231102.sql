...
--인라인뷰
--as 있던 없던 인라인뷰 안에서 선언된 alias도 사용할 수 있다.
SELECT
                사번, 성명
FROM (
               SELECT emp_id  "사번", emp_name  "성명" 
...
--왜냐하면 테이블 구조에 있는 데이터를 읽는 것만 가능한 구문임
--from절 아래에서 사용된 컬럼명은 select문 뒤 컬럼이 오는 자리에 사용가능함
--인라인 뷰도 from절 아래에서 사용된 집합이니 여기에서 사용된 별칭은 주 쿼리에서 사용가능함
--별칭을 줄 때 as를 붙이든 말든 모두 가능함
